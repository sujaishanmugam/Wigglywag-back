...
router.post("/verify",async (req, res, next) => {
  ...
  console.log(user);
    ...
    {
       resetUrl = `https://exoclinic.herokuapp.com/verifyemail/${verifyToken}`;
    ...
    // HTML Message
    const message = `
      <h1>You have requested a verify reset</h1>
      <p>Please make a put request to the following link:</p>
      <a href=${resetUrl} clicktracking=off>${resetUrl}</a>
    `;
...
//@access           public
router.post('/', (req,res) => {
   const{ name, email, password, userType, isFirstTime, isApproved, 
    phone, image, isFirstRegister} = req.body;

    console.log(isFirstRegister);
   //Validation
